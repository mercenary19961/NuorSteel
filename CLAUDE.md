...
### SEO — Sitemap + Robots.txt (DONE, 2026-04-26)
- [x] **Dynamic `/sitemap.xml`** generated by [SitemapController](app/Http/Controllers/SitemapController.php) + [resources/views/sitemap.blade.php](resources/views/sitemap.blade.php). Emits all 7 static public pages (home, about, products, quality, sustainability, career, contact) at top-priority tiers, then iterates `Product::active()->ordered()` and `CareerListing::open()->ordered()` to append each detail page. `<lastmod>` uses the model's `updated_at` so search engines see fresh timestamps when content changes; static pages emit `now()`. Per-page `<changefreq>` + `<priority>` tuned (home=1.0/weekly, products=0.9/monthly, contact=0.6/yearly etc.)
- [x] **Route at `/sitemap.xml`** in [routes/web.php](routes/web.php) — public, no auth, no rate limit (search engines need free access). Named `sitemap`. Returns `Content-Type: application/xml; charset=utf-8`
- [x] **Hardened [public/robots.txt](public/robots.txt)** — disallows `/admin`, `/admin/`, `/login`, `/forgot-password`, `/reset-password`, `/media/` (uploaded files shouldn't pollute search results); adds `Sitemap: https://nuorsteel.com/sitemap.xml` reference. Previous robots.txt was `Disallow:` (allow-everything) with no sitemap pointer
- [x] **Certificates route intentionally excluded** from the sitemap — the public `/certificates` route is currently commented out in `routes/web.php` (admin panel still manages the data, but the public page is hidden). When re-enabled, add a `route('certificates')` entry to the static URL list in `SitemapController::index()`
- [x] **Hreflang/locale alternates NOT emitted** — the site uses session-based locale switching (one URL serves both EN and AR via `POST /locale/{locale}`), so there's no separate URL per language. Adding hreflang would be misleading. If we ever move to URL-prefixed locales (`/en/...`, `/ar/...`), revisit this and emit `xhtml:link rel="alternate" hreflang="..."` per URL
- [x] **Post-deploy follow-ups documented** in the SEO section above (verify `APP_URL` matches production domain, update hardcoded `Sitemap:` line in robots.txt if the live domain differs, submit to Google Search Console + URL-inspect key pages, hit `/sitemap.xml` in production once after deploy to confirm `<loc>` URLs are correct)

### Turnstile Client Flag + Login 429 Handling (DONE, 2026-04-26)
- [x] **Client-side feature flag for Turnstile** — every public form (Footer newsletter, Contact, Career, JobDetail, Admin Login) now reads `usePage().props.turnstileSiteKey` and renders the widget only when it's set. Submit-button gating (`disabled={processing || !token}`) is also flag-aware: when Turnstile is disabled the button enables on `processing` only. Mirrors the server-side no-op behavior in [TurnstileVerifier::isEnabled()](app/Services/TurnstileVerifier.php) so local dev / pre-launch deploys without `TURNSTILE_SECRET_KEY` work end-to-end without UI dead-ends
- [x] **`TurnstileVerifier` logs `siteverify` rejections** — Cloudflare returns `200 OK` with `success=false` + `error-codes` on hostname mismatch / expired / duplicated tokens. Previously silent (the form just failed validation with no diagnostic trail). Now writes a `Log::warning('Turnstile siteverify rejected', ['error_codes' => ..., 'hostname' => ...])` so we can debug "widget says Success! but server rejects" cases
- [x] **`/admin/login` 429 → inline error** — per-IP `throttle:10,15` was hitting users with Laravel's default 429 page (full white "Too Many Requests" screen) which boots them off the form. Added a `ThrottleRequestsException` render hook in [bootstrap/app.php](bootstrap/app.php) scoped to `POST /admin/login` that converts the 429 into `redirect()->route('login')` with `withErrors(['email' => "Too many login attempts from this network. Please try again in {$minutes} minute(s)."])` and `withInput($request->only('email'))`. Rounds `Retry-After` seconds up to whole minutes. Same UX shape the per-email throttle already produces — users now see one consistent error pattern regardless of which throttle layer fired

### Admin Password Visibility Toggle (DONE, 2026-04-26)
- [x] New reusable `<PasswordInput>` primitive at [resources/js/Components/ui/PasswordInput.tsx](resources/js/Components/ui/PasswordInput.tsx) — wraps a normal text/password input with a right-edge eye icon button that flips `type="password"` ↔ `"text"`. Self-contained `useState`, no external deps, works inside Inertia `useForm` flows
- [x] Adopted across every admin password field: [Admin/Login.tsx](resources/js/Pages/Admin/Login.tsx), [Admin/AcceptInvite.tsx](resources/js/Pages/Admin/AcceptInvite.tsx) (new + confirm), [Admin/ResetPassword.tsx](resources/js/Pages/Admin/ResetPassword.tsx) (new + confirm), [Admin/ChangePassword.tsx](resources/js/Pages/Admin/ChangePassword.tsx) (current + new + confirm), [Admin/Users.tsx](resources/js/Pages/Admin/Users.tsx) (create/edit user dialog). Net diff: replaced ~60 lines of duplicated password-field JSX across 5 pages with a single import
- [x] Pattern: passwords default to hidden; eye icon shows on focus and persists. Useful for catching typos while admin types a password they're about to send to a user (invite/reset flows) — without leaking shoulder-surf risk by default

### Admin Media — Bulk Delete + Folder Delete Modes (DONE, 2026-04-26)
- [x] **Multi-select bulk delete** — admin Media page now has a checkbox per card + a header "Select" mode toggle. Once items are selected, a sticky action bar shows count + "Delete selected" button which opens a type-`delete`-to-confirm modal. POSTs to new `POST /admin/media/bulk-delete` route → `MediaController::bulkDestroy` (validates `ids` array, max 200, soft-deletes via `$media->delete()` per row so the model's `deleting` event chain still fires)
- [x] **Bulk delete intentionally has no Undo** — the modal type-confirm IS the safety net for an explicit bulk action. Items remain soft-deleted in the `media` table and can be restored from there if needed (existing `forceDeleting` hook preserves physical files). Single-item delete keeps its 30-sec session UndoButton — that flow is for accidental clicks, this one isn't
- [x] **Delete folder — opt-in "delete files inside" mode** — `deleteFolder` request now accepts `delete_files: bool`. Default behavior unchanged (move files to "general"). When set: soft-deletes every file in the folder via the same per-row `$media->delete()` loop. UI adds a checkbox in the folder-delete confirm modal that requires the folder name be typed before the destructive option unlocks. Success message reflects which mode ran ("Folder X deleted along with N files." vs "Folder X deleted. Files moved to general.")
- [x] Files: [MediaController.php](app/Http/Controllers/Admin/MediaController.php), [Admin/Media.tsx](resources/js/Pages/Admin/Media.tsx), [routes/web.php](routes/web.php) (new `media.bulk-destroy` route)

### Admin Products Form — Hide Category + Sort Order (DONE, 2026-04-26)
- [x] Removed the **Category** dropdown and **Sort Order** number input from the admin Products create/edit form ([Admin/Products/Form.tsx](resources/js/Pages/Admin/Products/Form.tsx)). Reasoning: the public Products page only renders TMT Bars + Billets and there's no roadmap to add more — the category taxonomy was unused, and the sort order is effectively fixed (TMT then Billets). Stripping the fields makes the form less intimidating for the client and removes two ways to break the page (e.g. typing "draft" in category → empty listing)
- [x] **Schema + controller untouched** — `category` + `sort_order` columns still exist on `products`, both controllers still set defaults on create. If the client ever needs more products, re-adding the inputs is a frontend-only change. Existing rows keep their values
- [x] Form file shrunk from ~71 LOC of edited JSX to ~24 LOC kept in the diff (a third of the form was these two fields' wrappers)

### LinkedIn URL Single-Source Fix (DONE, 2026-04-26)
- [x] [2026_04_26_120000_fix_linkedin_url_setting.php](database/migrations/2026_04_26_120000_fix_linkedin_url_setting.php) corrects the `linkedin_url` setting in DB — earlier seed migration set it to `https://linkedin.com/company/nuorsteel` (broken slug, no hyphen). The Footer reads from this setting; the homepage News section previously hardcoded the correct `https://www.linkedin.com/company/nuor-steel/`, leaving the two out of sync (footer link 404'd, news link worked)
- [x] Homepage News section ([resources/js/Pages/Public/Home.tsx](resources/js/Pages/Public/Home.tsx)) refactored to read from `siteSettings.linkedin_url` instead of hardcoded URL — single source of truth. Per-post LinkedIn buttons (which point to individual posts, not the company page) keep their hardcoded URLs since those are post-specific
- [x] [DemoContentSeeder](database/seeders/DemoContentSeeder.php) updated to seed the correct slug too, so a fresh DB starts with the right value
- [x] Pattern reminder: any URL surfaced in multiple components should pull from `siteSettings` (or i18n / page props) rather than be hardcoded — admins editing in `/admin/settings` need a single edit to update everything

### Admin Free-Text Fields — `dir=auto` (DONE, 2026-04-28)
- [x] Every admin field that displays user-submitted free text now sets `dir="auto"` so Arabic content renders right-to-left and English content stays left-to-right — without the admin having to toggle the panel locale. The browser detects directionality from the first strong character in the string
- [x] Reach: [Admin/Contacts.tsx](resources/js/Pages/Admin/Contacts.tsx) (subject + message), [Admin/Applications.tsx](resources/js/Pages/Admin/Applications.tsx) (cover-letter style fields), [Admin/ChangeLog.tsx](resources/js/Pages/Admin/ChangeLog.tsx) (diff payloads), [Admin/Dashboard.tsx](resources/js/Pages/Admin/Dashboard.tsx) (recent-activity previews)
- [x] Scoped to **user-submitted** content only — admin-controlled labels, headers, and CMS-editable English/Arabic side-by-side fields keep their explicit `dir` (the latter switches via `useLanguage()`). Don't blanket `dir=auto` everywhere — for known-locale strings it adds zero value and risks subtle layout shifts

### Public Forms — Autofill Background Override (DONE, 2026-04-28)
- [x] Chrome / Edge inject a yellow-cream `background-color` into autofilled inputs that overrode our `bg-white/5` translucent dark form fields on Contact + Career, making them visually pop out as opaque pale boxes inside an otherwise dark form
- [x] Fix in [resources/css/app.css](resources/css/app.css): `input:-webkit-autofill { -webkit-box-shadow: 0 0 0 1000px rgb(31 41 55 / 1) inset; -webkit-text-fill-color: white; transition: background-color 99999s ease-in-out 0s; }`. The 1000px box-shadow trick paints over the autofill background (you can't actually unset it directly), and the multi-decade transition delay prevents the brief yellow flash that browsers play during autofill. Color matches `bg-white/5` over the dark form surface within rounding error
- [x] Applied via a shared `autofill-dark` class added to dark-form inputs in [Contact.tsx](resources/js/Pages/Public/Contact.tsx) + [Career.tsx](resources/js/Pages/Public/Career.tsx). Light forms (admin login, etc.) don't need it — the default browser autofill color blends fine on white

### Public RTL Polish + Header Nav Sizing (DONE, 2026-04-29)
- [x] **Core Values icon parity** — People & Teamwork and Trust & Integrity (the two values added in the 4→6 expansion) now honor `imageBg` in BOTH the orbital-node view AND the detail-panel icon. Previously the orbital nodes got the orange `bg-primary` circle but the detail panel icon was missing the same treatment, producing visual inconsistency (4 of 6 cards had orange-backed icons in the detail view, 2 didn't). Fix in [radial-orbital-timeline.tsx](resources/js/Components/ui/radial-orbital-timeline.tsx): detail-panel icon wrapper checks `imageBg` and applies `bg-primary` + 3/4 image sizing to match
- [x] **Footer / Contact phone numbers — `dir="ltr"` on the anchor** — phone numbers like `+966543781868` were rendering with the leading `+` on the **right** in Arabic mode (because `+` is a neutral character that follows the surrounding paragraph direction). Forced `dir="ltr"` on the `<a href="tel:">` so the `+` always reads on the left of the digits. Files: [Footer.tsx](resources/js/Components/Layout/Footer.tsx), [Contact.tsx](resources/js/Pages/Public/Contact.tsx)
- [x] **Footer contact icons — logical margin** — swapped `mr-2` → `me-2` on the icon-text gap so the spacing mirrors correctly in RTL (icon was glued to the text in Arabic mode because physical `mr-2` doesn't flip)
- [x] **Header nav size tweak** — desktop nav links bumped from `text-xs` → `text-sm` while at the top of the page; reverts to `text-xs` once scrolled (compact mode unchanged). Two-character diff in [Header.tsx](resources/js/Components/Layout/Header.tsx); makes the navbar feel less cramped on first render

### Brand Rebrand to "حديد نور" + Hero Typewriter (DONE, 2026-04-29)
- [x] **Arabic brand name unified to "حديد نور"** (literally "Steel of Nuor") across every public surface — replaces the prior mix of "نور للحديد", "شركة نور للصناعات الحديدية", and other variants. Reach: home (hero, vision, mission, journey), about (overview, hero, journey), career (intro), quality (intro), products (TMT Bars features). Where the brand also appears in CMS-editable `site_content`, paired data migrations re-seed the row:
  - [2026_04_29_130000_update_about_arabic_copy_for_rebrand.php](database/migrations/2026_04_29_130000_update_about_arabic_copy_for_rebrand.php) — about overview + journey text
  - [2026_04_29_140000_rebrand_arabic_company_name_in_about_overview.php](database/migrations/2026_04_29_140000_rebrand_arabic_company_name_in_about_overview.php) — about overview separately (different SiteContent key)
  - [2026_04_29_120000_update_home_vision_mission_and_2030_arabic_copy.php](database/migrations/2026_04_29_120000_update_home_vision_mission_and_2030_arabic_copy.php) — home Vision/Mission/Vision 2030 Arabic copy
- [x] **i18n side**: Arabic strings updated in [resources/js/i18n/ar.ts](resources/js/i18n/ar.ts); a few English strings touched in [en.ts](resources/js/i18n/en.ts) (Partners → Collaborators rename) and [structured-data.blade.php](resources/views/partials/structured-data.blade.php) (matching brand name in Schema.org Organization JSON-LD). The legal English entity name "Nuor Steel Industry Company" is unchanged
- [x] **About hero rewrap** — Arabic title was a single long line that truncated awkwardly on narrow viewports; rewrapped into two lines via explicit line break in [About.tsx](resources/js/Pages/Public/About.tsx)
- [x] **Partners → Collaborators** — homepage + about-page section title renamed (EN + AR). The component file itself ([PartnersSection.tsx](resources/js/Components/Public/PartnersSection.tsx)) keeps its filename — the section is still commented out and the rename is content-only
- [x] **Hero typewriter cycle words** — the looping accent words after the static H1 line swapped from the old set (Sustainability / Quality / Strength / Heritage etc.) to **Sustainability / Leadership / Strength / Integration**. Reflects the four pillars the client wants to lead with. Files: [resources/js/i18n/en.ts](resources/js/i18n/en.ts), [ar.ts](resources/js/i18n/ar.ts), and a one-character touch in [Header.tsx](resources/js/Components/Layout/Header.tsx) (separate, accidental co-edit — not behavioral)
- [x] **Diacritics polish** — Arabic copy on Quality "أداء المتانة" and Products TMT Bars "التبريد المحكم" / Corrosion Resistance feature got tashkeel (vowel marks) added for cleaner pronunciation guidance. Includes data migration [2026_04_29_150000_polish_tmt_bars_features_arabic_copy.php](database/migrations/2026_04_29_150000_polish_tmt_bars_features_arabic_copy.php) for the products one (since features live in `product_specs`/seeded data, not just i18n)
- [x] Pattern note: text changes that affect both `site_content` AND i18n require BOTH a data migration AND an i18n edit — the existing CMS-content-wins-over-i18n rule means missing the seeder leaves stale text on prod even after a deploy. The Code Quality Checklist section already covers this; flagging here so future agents reviewing this section don't miss the dual-write requirement

### Product-Order Flip — TMT before Billets (DONE, 2026-05-06)
- [x] **Copy order swapped** so TMT/Steel Rebars now appears before Steel Billets in every "X and Y" sentence on home + about. Earlier copy listed billets first ("steel billets and TMT rebars"); client wanted the finished product (rebars) to lead. Affected strings: `home.about.description`, `about.overview.description`, `about.intro.highlight`, `about.timeline.events[2/3].body` (the 2023 + Present milestones)
- [x] Updated in [resources/js/i18n/en.ts](resources/js/i18n/en.ts) + [ar.ts](resources/js/i18n/ar.ts) (4 strings each), [SiteContentSeeder.php](database/seeders/SiteContentSeeder.php) (2 rows: home/about/description EN+AR, about/overview/description AR — the EN at this key is a different paragraph that didn't have the issue), and a new data migration [2026_05_06_120000_flip_tmt_rebars_before_steel_billets_in_copy.php](database/migrations/2026_05_06_120000_flip_tmt_rebars_before_steel_billets_in_copy.php) for prod DB
- [x] Note: the EN strings written by this migration's `up()` still used the term "TMT rebars". The subsequent Steel Rebars rename migration ([2026_05_06_130000](database/migrations/2026_05_06_130000_rename_tmt_bars_to_steel_rebars.php)) overwrites the same `home/about/description.content_en` row to use "Steel Rebars and Steel Billets" wording. Both migrations are idempotent and order-safe — running them in sequence produces the final state regardless of the prior DB state
- [x] One Arabic note flagged in the session: `about.overview.description` AR still uses the older "كتل الحديد" (literally "steel blocks") instead of the rebrand-standardized "ستيل بِليت" or "البليت". The Apr 29 rebrand migrations deliberately kept "كتل الحديد" in this paragraph, so it's intentional. Worth revisiting if the client mentions it

### Steel Rebars Rebrand — TMT Bars/Rebars Renamed Site-Wide (DONE, 2026-05-06)
- [x] **Sweeping rename**: "TMT Bars" / "TMT Rebars" / "TMT bar brand" → **"Steel Rebars"** / **"Steel Rebar"** across every user-facing surface, plus the product slug, image filenames, PDF filename, storage media filename, and i18n key paths
- [x] **Slug**: `tmt-bars` → `steel-rebars` on the `products` table. URL `/products?product=steel-rebars`. **Old-URL fallback unchanged**: [Products.tsx:413](resources/js/Pages/Public/Products.tsx#L413) does `products.find(p => p.slug === urlParams.get('product'))?.slug || products[0]?.slug`, so a stale `?product=tmt-bars` URL lands on the first product (Steel Rebars by sort order). No 301 redirect added since the site hasn't gone live yet — clean slate at production deploy
- [x] **Code references to slug `'tmt-bars'`** updated in [Products.tsx](resources/js/Pages/Public/Products.tsx) (`productDocuments` registry key, `localRenderSlugs`, `heroVariantSlugs`, two `selectedSlug === 'tmt-bars'` checks, comment), [Home.tsx](resources/js/Pages/Public/Home.tsx) (link href + product card image paths), [Header.tsx](resources/js/Components/Layout/Header.tsx) (Products mega menu link + image), [DemoContentSeeder.php](database/seeders/DemoContentSeeder.php) (slug + category fields)
- [x] **i18n key rename**: `home.products.tmtBars` → `home.products.steelRebars` (consumed by Home.tsx + Header.tsx) AND second namespace `products.tmtBars` → `products.steelRebars` (currently dead but renamed for consistency since slug-based dynamic lookup might come back). Header.tsx local label registry key `tmtDesc` → `steelRebarsDesc`
- [x] **Image + PDF filename renames** (`git mv`-tracked):
  - `public/images/home/products/{tmt-bars → steel-rebars}-{desktop,mobile}.png` (homepage product cards)
  - `public/images/products/renders/{tmt-bars → steel-rebars}-{en,ar}.webp` (Products page renders)
  - `public/images/products/renders/{tmt-bars → steel-rebars}-hero-{en,ar}.webp` (tagged Explore More variant)
  - `public/documents/products/{tmt-bars → steel-rebars}-material-test-certificate.pdf` (Overview tab PDF viewer)
  - `storage/app/private/media/{tmt-bars → steel-rebars}-product.png` (DB-tracked media row)
- [x] **All hardcoded image paths updated** in [Products.tsx](resources/js/Pages/Public/Products.tsx), [Home.tsx](resources/js/Pages/Public/Home.tsx), [Header.tsx](resources/js/Components/Layout/Header.tsx), [app.blade.php](resources/views/app.blade.php) (preload hint), [StorageReconciler.php](app/Services/StorageReconciler.php) (public source path + media destination)
- [x] **Arabic note**: where Arabic strings had a Latin "TMT" suffix tacked on (e.g. "حديد التسليح TMT" in `about.intro.highlight` and the 2023 timeline body), the Latin TMT was dropped — "حديد التسليح" already means reinforcement steel/rebar in Arabic, no replacement needed. Product title in Arabic was already "حديد التسليح" so no AR product name change
- [x] **Mission statement** ("leading TMT bar brand in the Middle East") updated to "leading Steel Rebar brand" — singular, since "brand" is singular. Two i18n locations + 2 site_content rows (home/vision_mission/mission_description, about/mission/description)
- [x] **Product description** ("Our TMT Bars are manufactured using advanced Thermo-Mechanical Treatment technology…") updated to "Our Steel Rebars are manufactured using advanced Thermo-Mechanical Treatment technology…" — kept the technical "Thermo-Mechanical Treatment" wording intact (it's the actual manufacturing process, not the product name)
- [x] **Data migration**: [2026_05_06_130000_rename_tmt_bars_to_steel_rebars.php](database/migrations/2026_05_06_130000_rename_tmt_bars_to_steel_rebars.php) — atomically updates the `products` row (slug + category + name_en + short_description_en/ar + description_en/ar), three `site_content` rows (home/about/description content_en, home/vision_mission/mission_description content_en, about/mission/description content_en), and the `media` row (path + filename + original_filename + alt_text_en). Reverse migration restores all old values
- [x] **Migration gotcha**: first run failed with `Unknown column 'updated_at' in field list` on the `media` update — the [media table](database/migrations/2026_02_05_000004_create_media_table.php) has `created_at` only by design, no `updated_at`. Fixed by removing the `updated_at` key from the media-update payload. Same gotcha as the `site_content` table (which is also `updated_at`-only). Pattern: any data migration writing to `media` or `site_content` via `DB::table()` must skip `updated_at` for `media` and skip `created_at` for `site_content`
- [x] **Files touched in this rename**: 12+ source files + 8 file renames + 2 seeders + 1 new migration. Internal `TmtMarkingDots` React component name + comment updated to `Steel Rebar Marking Dots` for consistency. Migration filenames + historical migrations (`2026_03_03`, `2026_04_14`, `2026_04_25`, `2026_04_29`) intentionally left alone — they ran with the old slug, modifying them would break Laravel's migration tracking

### Misc 2026-05-06 Polish (DONE, 2026-05-06)
- [x] **About Capabilities scroll-driven reveal shortened** — wrapper height in [CapabilitiesSection.tsx:95](resources/js/Components/Public/CapabilitiesSection.tsx#L95) dropped from `350vh` → `250vh`. Total scroll distance through the section is now ~150 vh (down from ~250 vh) with each card animating over ~25 vh / ~230 px (down from ~42 vh / ~380 px). Felt too slow at 350vh; 250vh is snappy without making cards flash by. Mobile path (`isDesktop ? '350vh' : 'auto'`) unaffected — mobile uses time-based flip-in animation, not scroll-driven
- [x] **Quality hero manufacturing-process PDF replaced** — file at [public/documents/quality/manufacturing-process.pdf](public/documents/quality/manufacturing-process.pdf) swapped from the original 152 KB version to a new 952 KB "overview of SMS&RSM Manufacturing process-1 (1).pdf" sourced from the client. Filename kept the same so the Quality hero CTA continues to work without any code change. Cloudflare cache caveat: static assets are keyed without query strings on Cloudflare — to refresh after deploy, purge the URL via the dashboard or hard-reload the page
- [x] **Quality hero text vertical alignment** — bottom-padding on the flex-centered hero copy in [Quality.tsx:114](resources/js/Pages/Public/Quality.tsx#L114) reduced from `pb-32 lg:pb-44` → `pb-4 lg:pb-12` (text now sits closer to true viewport center; the `pb-32 lg:pb-44` asymmetric offset was pushing content too high on laptop screens). Same `pb-32 lg:pb-44` is still in use on [Career.tsx:149](resources/js/Pages/Public/Career.tsx#L149) — flag if you want them visually consistent
...
> **Last updated:** 2026-05-09 — **Catch-up doc sync covering 5 commits on 2026-05-06.** Three tracks: (1) **Product-order flip** — TMT/Steel Rebars now leads "X and Y" sentences across home + about, ahead of Steel Billets. Migration [2026_05_06_120000_flip_tmt_rebars_before_steel_billets_in_copy.php](database/migrations/2026_05_06_120000_flip_tmt_rebars_before_steel_billets_in_copy.php) flips the order in `home/about/description` (EN+AR) and `about/overview/description` (AR only) site_content rows. Same flip applied to 4 i18n strings each in en.ts + ar.ts. The migration's `up()` originally wrote "TMT rebars" copy; the subsequent rename migration overwrites the EN row with "Steel Rebars and Steel Billets" wording — both migrations are idempotent so order doesn't matter. (2) **Steel Rebars rebrand** — sweeping rename of "TMT Bars/Rebars" → "Steel Rebars" across every user-facing surface: product slug `tmt-bars` → `steel-rebars` (+ category field), product `name_en` "TMT Bars" → "Steel Rebars", product description (drops "TMT" while keeping "Thermo-Mechanical Treatment" technical wording), all 9 affected EN i18n strings + 4 AR i18n strings (Latin "TMT" suffix dropped from Arabic since "حديد التسليح" already means rebar), i18n key namespaces `home.products.tmtBars` → `home.products.steelRebars` and dead second namespace `products.tmtBars` → `products.steelRebars`, Header.tsx local `L.products.tmtDesc` key → `steelRebarsDesc`, mission statement "TMT bar brand" → "Steel Rebar brand" in 2 i18n locations + 2 site_content rows, plus `git mv` renames of 8 image/PDF files (homepage product cards `tmt-bars-{desktop,mobile}.png`, Products page renders `tmt-bars-{en,ar}.webp` + `tmt-bars-hero-{en,ar}.webp`, datasheet PDF `tmt-bars-material-test-certificate.pdf`, storage media `tmt-bars-product.png`). All hardcoded paths updated in Products.tsx / Home.tsx / Header.tsx / app.blade.php / StorageReconciler.php / DemoContentSeeder.php. Data migration [2026_05_06_130000_rename_tmt_bars_to_steel_rebars.php](database/migrations/2026_05_06_130000_rename_tmt_bars_to_steel_rebars.php) atomically updates the products row + three site_content rows + the media row path. **Migration gotcha**: first run failed with `Unknown column 'updated_at'` on the `media` update — the media table has `created_at` only by design (same pattern as `site_content`). Fixed by removing the timestamp from the media payload; documented as a recurring trap (any data migration writing to `media` or `site_content` via `DB::table()` must skip `updated_at` / `created_at` respectively). Old-slug fallback: `Products.tsx:413` falls through to `products[0]?.slug` on URL slug mismatch, so any stale `?product=tmt-bars` URL lands on the first product (Steel Rebars by sort order); no 301 redirect added since the site hasn't been published live yet — clean slate at deploy. Internal `TmtMarkingDots` React component + comment renamed too. Historical migrations (Mar/Apr) intentionally left alone — they ran with the old slug, modifying would break Laravel's migration tracking. (3) **Misc polish**: About Capabilities scroll-driven reveal duration shortened — wrapper height in [CapabilitiesSection.tsx:95](resources/js/Components/Public/CapabilitiesSection.tsx#L95) dropped from `350vh` → `250vh`, total scroll distance ~250 vh → ~150 vh, ~25 vh per card (was ~42 vh per card); mobile path unaffected. Quality hero manufacturing-process PDF replaced with new 952 KB "SMS&RSM Manufacturing process-1" sourced from client (filename preserved as `manufacturing-process.pdf` so no code change needed; Cloudflare cache caveat — static assets keyed without query strings, purge URL or hard-refresh after deploy). Quality hero text vertical alignment iterated — bottom-padding [Quality.tsx:114](resources/js/Pages/Public/Quality.tsx#L114) `pb-32 lg:pb-44` → `pb-4 lg:pb-12` after user feedback that the asymmetric offset pushed content too high on laptop. Career page [Career.tsx:149](resources/js/Pages/Public/Career.tsx#L149) still uses the original `pb-32 lg:pb-44` — flag if visual consistency between Quality + Career heroes matters. Prior (2026-04-29): **Catch-up doc sync covering 9 commits between 2026-04-26 and 2026-04-29.** Five pieces from 2026-04-26: (1) **Turnstile client feature flag** — every public form (Footer / Contact / Career / JobDetail / Admin Login) reads `siteSettings.turnstileSiteKey` and renders the widget + gates submit-disabled only when set, mirroring [TurnstileVerifier::isEnabled()](app/Services/TurnstileVerifier.php) so dev / pre-launch deploys without `TURNSTILE_SECRET_KEY` work end-to-end. Plus `Log::warning` on Cloudflare's `200 OK + success=false` responses (hostname/expired/duplicated tokens) so siteverify rejections leave a diagnostic trail. Plus a `ThrottleRequestsException` render hook in [bootstrap/app.php](bootstrap/app.php) scoped to `POST /admin/login` that converts the per-IP `throttle:10,15` 429 into an inline `withErrors(['email' => "Too many login attempts..."])` redirect — same UX shape as the per-email throttle, no more being booted to Laravel's default 429 page. (2) **Reusable `<PasswordInput>`** primitive ([resources/js/Components/ui/PasswordInput.tsx](resources/js/Components/ui/PasswordInput.tsx)) with eye-icon visibility toggle, adopted across [Login](resources/js/Pages/Admin/Login.tsx), [AcceptInvite](resources/js/Pages/Admin/AcceptInvite.tsx), [ResetPassword](resources/js/Pages/Admin/ResetPassword.tsx), [ChangePassword](resources/js/Pages/Admin/ChangePassword.tsx), and [Users](resources/js/Pages/Admin/Users.tsx) — replaces ~60 lines of duplicated password-field JSX with one import. Useful for catching typos when admins type a password they're about to send (invite/reset flows). (3) **Admin Media bulk delete + folder-with-files mode** — multi-select checkboxes per card + sticky action bar, type-`delete`-to-confirm modal, new `POST /admin/media/bulk-delete` route → [MediaController::bulkDestroy](app/Http/Controllers/Admin/MediaController.php) (validates `ids` array, max 200). Bulk delete intentionally has no Undo (the type-confirm IS the safety net for an explicit bulk action; soft-deletes are still recoverable from the `media` table via `forceDeleting` hook). Folder delete extended with opt-in `delete_files` boolean — checkbox in the folder-delete confirm modal lets admin soft-delete every file inside the folder; default still moves them to "general". (4) **Admin Products form trimmed** — Category dropdown + Sort Order input removed from create/edit form ([Admin/Products/Form.tsx](resources/js/Pages/Admin/Products/Form.tsx)). Both columns + controllers untouched (still set defaults on create) — purely UI cleanup since the public site only ships TMT Bars + Billets and there's no roadmap for more products. Re-adding the inputs is a frontend-only change. (5) **LinkedIn URL fixed in DB** — [2026_04_26_120000_fix_linkedin_url_setting.php](database/migrations/2026_04_26_120000_fix_linkedin_url_setting.php) corrects `linkedin_url` from the broken `linkedin.com/company/nuorsteel` slug (no hyphen, 404'd) to `https://www.linkedin.com/company/nuor-steel/`. Homepage News section refactored to read from `siteSettings.linkedin_url` instead of hardcoded URL — single source of truth so admins editing in `/admin/settings` update both Footer + Home in one shot. Two pieces from 2026-04-28: **`dir="auto"` on user-submitted free-text** ([Admin/Contacts.tsx](resources/js/Pages/Admin/Contacts.tsx) subject + message, [Admin/Applications.tsx](resources/js/Pages/Admin/Applications.tsx), [Admin/ChangeLog.tsx](resources/js/Pages/Admin/ChangeLog.tsx) diff payloads, [Admin/Dashboard.tsx](resources/js/Pages/Admin/Dashboard.tsx) recent-activity previews) — browser detects directionality from first strong character so Arabic submissions render RTL without admin needing to flip the panel locale; scoped to user-submitted content only (admin labels keep their explicit `dir`). And **autofill background override** for Contact + Career dark forms — Chrome / Edge inject a yellow-cream background into autofilled inputs that overrode `bg-white/5`; fixed via the classic `-webkit-box-shadow: 0 0 0 1000px rgb(31 41 55) inset` trick + `transition-delay: 99999s` to suppress the flash, applied via shared `autofill-dark` class. Two pieces from 2026-04-29: **Public RTL polish + header sizing** — Core Values People & Trust now honor `imageBg` in BOTH orbital node + detail panel icon (was inconsistent — 4 of 6 had orange-backed icons, 2 didn't), forced `dir="ltr"` on Footer + Contact phone anchors so the leading `+` reads on the left in Arabic mode, swapped `mr-2` → `me-2` on Footer contact icons for correct RTL mirroring, desktop nav `text-xs` → `text-sm` while at top (compact mode unchanged). And **Arabic brand rebrand to "حديد نور"** — unified across home / about / career / quality / products (Arabic only; English entity name "Nuor Steel Industry Company" unchanged). Where the brand appears in `site_content`, paired data migrations re-seed the rows ([2026_04_29_120000](database/migrations/2026_04_29_120000_update_home_vision_mission_and_2030_arabic_copy.php) home Vision/Mission/Vision 2030, [2026_04_29_130000](database/migrations/2026_04_29_130000_update_about_arabic_copy_for_rebrand.php) about, [2026_04_29_140000](database/migrations/2026_04_29_140000_rebrand_arabic_company_name_in_about_overview.php) about overview, [2026_04_29_150000](database/migrations/2026_04_29_150000_polish_tmt_bars_features_arabic_copy.php) TMT Bars features) — reminder per the Code Quality Checklist that text appearing in BOTH i18n AND `site_content` requires BOTH a `.ts` edit AND a data migration; missing the seeder leaves stale text on prod. Hero typewriter cycle words swapped to **Sustainability / Leadership / Strength / Integration** (the four pillars the client wants to lead with). "Partners" → "Collaborators" rename across home + about (EN + AR; component filename `PartnersSection.tsx` unchanged since the section is still commented out). About-page Arabic hero rewrapped into two lines (was truncating on narrow viewports). Diacritics added to Quality "أداء المتانة" and Products TMT Bars "التبريد المحكم" / Corrosion Resistance feature copy. Prior (2026-04-26 morning): **SEO sitemap + robots.txt hardening.** Added [SitemapController](app/Http/Controllers/SitemapController.php) + [resources/views/sitemap.blade.php](resources/views/sitemap.blade.php) serving a dynamic `/sitemap.xml` that lists all 7 static public pages plus every active product detail page and every open career listing, with `<lastmod>` driven by the model `updated_at`. Route registered in [routes/web.php](routes/web.php) — public, no auth, no rate limit (search engines need free access). [public/robots.txt](public/robots.txt) was previously a permissive `Disallow:` placeholder with no sitemap pointer; rewritten to disallow `/admin`, `/admin/`, `/login`, `/forgot-password`, `/reset-password`, `/media/` and to reference `Sitemap: https://nuorsteel.com/sitemap.xml`. Certificates intentionally excluded from the sitemap (public route is currently commented out — re-add to the static URL list when the page is re-enabled). Hreflang alternates not emitted because locale switching is session-based (one URL per page serves both EN and AR via `POST /locale/{locale}`); revisit if we ever move to `/en/...` `/ar/...` URL-prefixed locales. Five post-deploy TODOs added to the SEO section: (1) verify `APP_URL` on Railway matches the production domain so `route()` emits canonical URLs, (2) update the hardcoded `Sitemap:` line in `robots.txt` if the live domain differs from `nuorsteel.com`, (3) submit to Google Search Console (DNS-verify, submit sitemap, URL-inspect key pages), (4) hit `/sitemap.xml` in production once after deploy to confirm `<loc>` URLs use the right host, (5) optional: also submit to Bing Webmaster Tools. Prior (2026-04-25 evening): Two big tracks: (A) **Auth + account security hardening** in 8 thin slices, end-to-end. New schema fields on users (`must_change_password`, `password_changed_at`, `last_login_at`, `last_login_ip`, `invited_at`). New rule: admin accounts are self-managed, another admin cannot edit/delete/toggle them (UI shows a Lock + "Self-managed" pill instead of the action buttons). New public forgot-password flow at `/forgot-password` → `/reset-password/{token}` with a themed [PasswordResetEmail](app/Mail/PasswordResetEmail.php) Mailable. New admin invite flow: radio in Create User form picks "Send invite email" (default) vs "Set password now"; invite path uses `URL::temporarySignedRoute` for a 48h signed accept URL (no token table needed) and ships the themed [AdminInviteEmail](app/Mail/AdminInviteEmail.php). `must_change_password` flag is set when admin types a password manually, then the [ForcePasswordChange middleware](app/Http/Middleware/ForcePasswordChange.php) intercepts every admin route and redirects to `/admin/change-password` until the user picks a new one. New [AdminIdleTimeout middleware](app/Http/Middleware/AdminIdleTimeout.php) logs out the admin after 30 min idle (constant at the top is bump-able). Login throttle layered: per-IP `throttle:10,15` on the route + per-email 5/15min in [LoginController](app/Http/Controllers/Auth/LoginController.php) + 750ms `usleep` on every failed attempt. `last_login_at` + `last_login_ip` stamped on success and surfaced in the admin users list as a "Last login" column ("3 hr ago, From 1.2.3.4"). "Pending invite" amber status pill + Mail-icon resend button on rows where the invitee hasn't logged in yet. Mail config in [.env.example](.env.example) reshaped for **Resend SMTP** (free tier: 100/day, 3,000/month) — local dev stays on `MAIL_MAILER=log`, production toggles to `smtp` once the API key + domain DNS verification land. Files added: 4 controllers, 2 mailables, 2 blade templates, 2 middleware, 4 Inertia pages (ForgotPassword, ResetPassword, AcceptInvite, ChangePassword), 2 migrations. (B) **Footer social media links** — Facebook + Instagram added alongside LinkedIn as 3 circular icon buttons. Both URLs admin-editable as `facebook_url` + `instagram_url` Settings keys (parallel to `linkedin_url`); seeded via [2026_04_25_130000_seed_facebook_instagram_settings.php](database/migrations/2026_04_25_130000_seed_facebook_instagram_settings.php), shared via `HandleInertiaRequests`, type-extended in `PageProps.siteSettings`, icon-mapped in admin Settings page (renders automatically since the admin Settings page iterates over the `contact` group). Earlier 2026-04-25 (afternoon batch — already documented in prior sections): manufacturing-process PDF viewer on Quality hero, in-page datasheet viewer on Products Overview, TMT Bars chemical composition tab, spec table scroll-down hint pill + `.scrollbar-bright` CSS variant, billets spec split into Physical + Chemical tabs, Products full-white text sweep + 4s auto-cycling tabs, Sustainability governance pillars + safety grid centering. Prior (2026-04-25 afternoon overview): Four-part follow-up: (1) **Spec table scroll affordance** — added a "More" pill at the bottom-right of `SpecDataTable` (`ChevronDown` + `animate-scroll-hint` keyframe, 4px down + opacity 0.7→1 over 1.4s) that appears only when there's vertical overflow AND the user hasn't scrolled to the bottom; clicking it smooth-scrolls ~70% of viewport-height. Plus a `linear-gradient(to top, rgba(0,0,0,0.25), transparent)` bottom fade. New `.scrollbar-bright` CSS variant in [app.css](resources/css/app.css) — white-at-60% thumb + white-at-12% track, 8px wide, **always visible** (no hover-gate); scoped to spec tables since other modals (career, certificates) keep using the auto-hiding `scrollbar-thin`. (2) **TMT Bars chemical composition tab** seeded via [2026_04_25_120000_seed_tmt_bars_chemical_composition.php](database/migrations/2026_04_25_120000_seed_tmt_bars_chemical_composition.php) — typical C/S/P/Mn per Ø10–Ø20 sourced from the 6 Mill Test Certificates in `MTC 2026.pdf` (ASTM E415/2021). Headers call out the ASTM A615+A1/21 phosphorus ceiling. Si + larger sizes (Ø22/25/28/32) flagged as follow-up — the certificate template doesn't include Si and only ships Ø10–Ø20 today. The `spec_table_2` plumbing built earlier for billets handles this automatically — no frontend code changes for the chemical tab itself. (3) **In-page datasheet PDF viewer** on Products Overview tab — new `productDocuments` registry mapping slug → `{ url, labelEn, labelAr }`; "View Specification Sheet" / "View Material Test Certificate" button placed below the description in white-pill style. `viewingDoc` state + `AnimatePresence` modal hosts the iframe. **Toolbar-clip / download-hide attempt was reverted** — the iframe positioning broke layout in the Sustainability + Products modals, and `#toolbar=0` is unreliable in Chrome anyway; final decision is to leave the browser PDF toolbar visible (deterrent-only hiding is not worth the layout fragility). (4) **Manufacturing Process PDF on Quality hero** — new CTA below the subtitle that opens the `manufacturing-process.pdf` (sourced from the client's `Manufacturing Procress Nuor Steel 2026.pdf`, typo preserved in source filename only). Button uses the **homepage hero CTA shimmer style** (orange outline, `animate-cta-shimmer` keyframe, sliding `bg-primary` fill on hover, `ArrowRight` icon, RTL-aware) instead of the white-pill style from products — visual consistency: all primary public CTAs now share the same button language. Side benefit: refactored Quality's `viewingPdf` from `string | null` → `{ url, title } | null` so one modal hosts both showcase certificates and the manufacturing-process PDF (eliminated the URL→entry lookup). Earlier 2026-04-25 (3 commits before this batch): (1) Billets spec split into Physical + Chemical tabs via `spec_table_2` JSON column and `tab_label_en/ar` fields; (2) Products full-white text sweep + 4s auto-cycling tabs with gradient fill; (3) Sustainability governance pillars + safety grid centering + NCEC em dash strip. Prior (2026-04-23): Client handoff docs — [docs/ADMIN_PANEL_GUIDE.md](docs/ADMIN_PANEL_GUIDE.md) + .docx + new client-facing [README.md](README.md) + portable [docs/magic-card-prompt.md](docs/magic-card-prompt.md); stale `PLAN.md` removed. Prior (2026-04-22): Admin UX hardening across Partners (logo fill + name-confirm delete), Products (eager-load fallback + featured_image reseed migration), Settings (dirty-state Modified badges + email chip type-`delete`-confirm), Change Log (type-`delete`-confirm), Applications (status-aware sort + `viewed_at` column + brand-orange new pulse). Also codified `## Collaboration — Commit Message Suggestions` end-of-turn convention. Prior (2026-04-21): TMT bar hero variant swap + 6 orange pulsing marking dots (`TmtMarkingDots`) in Explore More, per-product hero sizing restored via arbitrary `min-[1800px]:` variant (Vite HMR doesn't reliably pick up new `@theme` breakpoint tokens), fixed migration inserting non-existent `created_at` into `site_content`. Prior (2026-04-20): Header redesign — transparent-to-glass navbar, three dropdowns (About / Products mega / Certificates), 150ms close delay on desktop, mobile accordion. Prior (2026-04-19): public-surface security hardening — Turnstile on all public POST forms, CSP rebuilt, `trustProxies` locked to Cloudflare CIDRs, login throttle layered, LinkedIn URL host/scheme validation, SVG uploads blocked from public serving.
