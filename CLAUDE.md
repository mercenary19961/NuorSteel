...
> **Last updated:** 2026-04-25 — Three-part session: (1) **Billets specifications restructured into Physical + Chemical tabs** — new `spec_table_2` JSON column on `products` (parallel shape to `spec_table`); migration moves the existing Chemical Composition into `spec_table_2` and replaces `spec_table` with a 14-row Physical Properties Reference (with `rows_ar` for Arabic property names, since property labels are now bilingual not just headers). New `tab_label_en/ar` JSON fields drive semantic tab labels ("Physical Properties" / "Chemical Composition") in place of the generic "Specifications" — labels travel with the spec data, not via i18n, since they're content-shaped. Public ProductController + types + Admin validation/fillable/jsonFields + Product model fillable+cast all updated end-to-end. **Admin Form.tsx editing UI for `spec_table_2` / `tab_label_*` / `rows_ar` not yet built — DB/migration-only**; backend already accepts these fields so the UI can be added later without controller changes. TMT Bars unaffected (single `spec_table`, no `tab_label_*` → falls through to default i18n label, only one specs tab renders). (2) **Products page UX polish**: every reduced-opacity content text (`text-white/{60,70,80,90}`) across all tabs and both products bumped to solid `text-white` because the bright orange left panel was washing out anything below 100%. Spec table dividers bumped `white/15 → white/25` for the same reason. Tabs now auto-cycle every 4s while in expanded "Explore More" view via a `requestAnimationFrame` loop driving a `cycleProgress` state (0..1), with the next tab gradually morphing into the active style — white overlay opacity in, text color interpolated white→gray-900 via inline `rgb()` math, border alpha `white/30 → transparent`. Manual click pauses the cycle so users can read; cycle resumes on product switch or re-opening Explore More. Tab button restructured with relative-overflow container + absolute fill span + relative label span so the fill animates underneath without re-layout. (3) **Sustainability polish**: Governance "Coming Soon" placeholder replaced with three MagicCard pillars (Board Oversight, Ethics & Integrity, Regulatory Compliance) + `ShieldCheck` commitment footer. Safety tab grid centers the two-PDF case via dynamic `max-w-4xl` (was always `lg:grid-cols-3 max-w-6xl`, left-skewed when only Safety Policy + Emergency Procedures present). Em dash stripped from NCEC EN description. Prior (2026-04-23): Client handoff docs — [docs/ADMIN_PANEL_GUIDE.md](docs/ADMIN_PANEL_GUIDE.md) + .docx + new client-facing [README.md](README.md) + portable [docs/magic-card-prompt.md](docs/magic-card-prompt.md); stale `PLAN.md` removed. Prior (2026-04-22): Admin UX hardening across Partners (logo fill + name-confirm delete), Products (eager-load fallback + featured_image reseed migration), Settings (dirty-state Modified badges + email chip type-`delete`-confirm), Change Log (type-`delete`-confirm), Applications (status-aware sort + `viewed_at` column + brand-orange new pulse). Also codified `## Collaboration — Commit Message Suggestions` end-of-turn convention. Prior (2026-04-21): TMT bar hero variant swap + 6 orange pulsing marking dots (`TmtMarkingDots`) in Explore More, per-product hero sizing restored via arbitrary `min-[1800px]:` variant (Vite HMR doesn't reliably pick up new `@theme` breakpoint tokens), fixed migration inserting non-existent `created_at` into `site_content`. Prior (2026-04-20): Header redesign — transparent-to-glass navbar, three dropdowns (About / Products mega / Certificates), 150ms close delay on desktop, mobile accordion. Prior (2026-04-19): public-surface security hardening — Turnstile on all public POST forms, CSP rebuilt, `trustProxies` locked to Cloudflare CIDRs, login throttle layered, LinkedIn URL host/scheme validation, SVG uploads blocked from public serving.
